...
if ! grep -q "100[0-9]:100[0-9]" /etc/passwd; then
	...
	# copy lxpanel configs
	if [ ! -d /home/$USER/.config/lxpanel/slitaz ]; then
		mkdir -p /home/$USER/.config/lxpanel
		cp -a /etc/lxpanel/slitaz /home/$USER/.config/lxpanel
		chown -R $USER.users /home/$USER/.config/lxpanel
		chown -R 700 /home/$USER/.config/lxpanel
	fi
fi
